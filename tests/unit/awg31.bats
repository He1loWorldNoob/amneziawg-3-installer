...
@test "server-init не пересоздаёт живой интерфейс" {
    # Пересоздание — это iface remove и iface add. Единая команда «пересоздать
    # с переносом пиров» меняла ещё и ключи сервера с параметрами обфускации,
    # так что перенесённые пиры оказывались мёртвым списком: подключиться по
    # ним было нельзя ничем.
    ! grep -q "SRV_FORCE" "$REPO_ROOT/awg3.sh"
    ! grep -q "peers_src" "$REPO_ROOT/awg3.sh"
    ! grep -q "extract_peers" "$REPO_ROOT/awg3.sh"
    local body
    body=$(sed -n '/^guard_existing_server() {/,/^}/p' "$REPO_ROOT/awg3.sh")
    [[ "$body" == *"iface remove"* ]]
}

@test "прежний порт снимается с фаервола только при смене" {
    # Правило переоткрывается при пересоздании интерфейса на другом порту;
    # на том же порту снимать и ставить заново — значит закрыть его самому
    # себе на мгновение.
    local body
