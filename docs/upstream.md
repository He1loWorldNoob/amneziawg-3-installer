...
| перезагрузки после шагов 1 и 2 | апстрим перезагружался **безусловно**, полагаясь на возобновление по `STATE_FILE`; у нас шаги идут подряд, поэтому ребут только при обновлении ядра либо когда `modprobe amneziawg` не проходит — иначе получался бесконечный цикл перезагрузок |
| выбор UDP-порта | `resolve_awg_port` разрешает порт **до** настройки фаервола: `step4_setup_firewall` открывает `${AWG_PORT}/udp`, а `server-init` выполняется позже, и при пустом `--awg-port` ufw получал `ERROR: Bad port` |
...
- **Имя хоста для клиентских `Endpoint`** хранится в `awg0.conf` строкой
  `#_Endpoint = имя` (по образцу `#_Name` у пиров): `server_endpoint_name`
  читает его, `resolve_endpoint` ставит вторым по приоритету после
  `--endpoint`, `server-init --force` сохраняет при пересоздании, `cmd_list`
  показывает.
- **`::/0` не попадает в `AllowedIPs`** клиенту без IPv6-адреса
  (`strip_ipv6_routes`): на машине с отключённым IPv6 `awg-quick` падал на
  этом маршруте с «IPv6 is disabled on nexthop device» и не поднимал туннель.
  Явный `--allowed-ips` уважается как есть.
- **`SCRIPT_DIR` разыменовывает симлинк** через `readlink -f`: скрипт
  вызывается как `/usr/local/sbin/awg3` и иначе искал бы клиентов в
  `/usr/local/sbin`.
