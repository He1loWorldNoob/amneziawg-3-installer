...
интерактивная панель, устроенная вглубь: **серверы → интерфейсы → ключи →
карточка ключа**. Каждый экран показывает краткий обзор уровня ниже — сколько
на сервере интерфейсов, сколько на интерфейсе ключей, какой у ключа адрес и
связь. Оттуда же создаются и удаляются интерфейсы и ключи, скачиваются `.conf`,
QR и ссылки `vpn://`, делаются бэкапы и перезапускается сервис.
...
- [**Установка от и до**](docs/install.md) — весь путь от пустого сервера до
  ключа в телефоне: требования, четыре шага с реальным выводом каждого,
  несколько интерфейсов, панель и что делать, если не пошло.
- [Руководство по использованию](docs/usage.md) — всё, что после установки:
  управление ключами, эксплуатация, смена параметров сервера, типовые
  проблемы, удаление, справочник команд.
- [Апстрим и наши правки](docs/upstream.md) — чем форк отличается от
  оригинала и как обновлять `vendor/`.
